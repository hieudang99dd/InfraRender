...
Trình duyệt: Next.js / React / TypeScript
  ...
  └─ API client → NEXT_PUBLIC_INFRARENDER_API_URL
          ↓ HTTPS + mã truy cập ứng dụng khi được yêu cầu
...
Khi lưu, backend kiểm tra ảnh được tham chiếu còn tồn tại. Khi xóa file, backend từ chối nếu một dự án vẫn sử dụng ảnh. Xóa dự án chỉ dọn ảnh nếu không còn dự án khác tham chiếu. Retention mặc định 30 ngày xét theo tuổi file, chỉ áp dụng ảnh không được tham chiếu; tác vụ chạy mỗi 24 giờ sau khi backend khởi động. API hỗ trợ xem trước và thực hiện dọn dẹp.
