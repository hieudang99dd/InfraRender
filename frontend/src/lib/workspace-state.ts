...
export function emptyWorkspace(defaults: RenderSettings): WorkspaceState {
  return {
    schemaVersion: 2,
    projectName: "Dự án hạ tầng mới",
    source: null,
    settings: { ...defaults, custom_keywords: [] },
    notes: "",
    prompt: "",
    negativePrompt: "",
    versions: [],
...
function text(value: unknown, max = 28000) {
...
function readSettings(value: unknown, defaults: RenderSettings): RenderSettings {
  ...
  for (const key of Object.keys(defaults) as (keyof RenderSettings)[]) {
    ...
    if (key === "custom_keywords") {
      if (Array.isArray(val))
        result.custom_keywords = val
          .filter((v): v is string => typeof v === "string" && [...v].length <= 120)
          .slice(0, 20);
    } else if (key === "creativity") {
      if (typeof val === "number" && Number.isInteger(val) && val >= 0 && val <= 100)
        result.creativity = val;
  ...
  if (
    result.aspect_ratio &&
    !/^(Original|[1-9][0-9]{0,3}:[1-9][0-9]{0,3})$/.test(result.aspect_ratio)
  )
    result.aspect_ratio = "";
  return result;
}
function readSource(value: unknown, backend: string): StoredSource | null {
  const data = record(value),
    name = text(data.saved_name, 100);
  if (!mediaName.test(name)) return null;
  return {
    saved_name: name,
    url: `${backend}/uploads/${name}`,
    name: text(data.name, 200) || name,
    size: text(data.size, 100),
    resolution: text(data.resolution, 100),
  };
}
export function normalizeWorkspace(
  ...
  const data = record(value),
    result = emptyWorkspace(defaults);
  ...
  result.prompt = text(data.prompt);
  result.negativePrompt = text(data.negativePrompt, 4000);
  result.notes = text(data.notes, 5000);
  result.generatedFrom = text(data.generatedFrom, 50000);
  if (["template", "refine", "vision"].includes(String(data.promptMode)))
    result.promptMode = data.promptMode as PromptMode;
  result.promptAnalysis = Array.isArray(data.promptAnalysis)
    ? data.promptAnalysis.filter((v): v is string => typeof v === "string").slice(0, 12)
    : [];
  ...
  if (Array.isArray(data.versions))
    result.versions = data.versions
      ...
      .map((v) => ({
        id: text(v.id, 200),
        createdAt: text(v.createdAt, 100),
        prompt: text(v.prompt),
        negativePrompt: text(v.negativePrompt, 4000),
        settings: readSettings(v.settings, defaults),
        notes: text(v.notes, 5000),
        sourceName: text(v.sourceName, 200),
        generatedFrom: text(v.generatedFrom, 50000),
  ...
  const rawRenderVersions =
    Array.isArray(data.renderVersions) && data.renderVersions.length > 0
      ? data.renderVersions
      : Array.isArray(data.renderHistory)
        ? data.renderHistory
        : [];

  if (rawRenderVersions.length > 0)
    result.renderVersions = rawRenderVersions
      .filter(
        (v) =>
          v &&
          typeof v.id === "string" &&
          mediaName.test(v.name) &&
          typeof v.prompt === "string" &&
      ...
      .map((v) => ({
        ...
        settings: readSettings(v.settings, defaults),
        notes: text(v.notes, 5000),
        projectName: text(v.projectName, 200),
        provider: text(v.provider, 200),
        model: text(v.model, 200),
        source: readSource(v.source, backend),
        details: {
          native_size: text(record(v.details).native_size, 50),
          final_size: text(record(v.details).final_size, 50),
          provider_size: text(record(v.details).provider_size, 50),
          processing: text(record(v.details).processing, 50),
          upscaled: record(v.details).upscaled === true,
          cropped: record(v.details).cropped === true,
          experimental: record(v.details).experimental === true,
        },
      }));
  result.activeVersion = result.versions.some((v) => v.id === data.activeVersion)
    ? (data.activeVersion as string)
    : null;
  result.activeRenderId = result.renderVersions.some((v) => v.id === data.activeRenderId)
    ? (data.activeRenderId as string)
    : null;
  return result;
}
