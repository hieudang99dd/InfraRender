...
export function useWorkspace() {
  ...
  const uploadRef = useRef<AbortController | null>(null),
    generationRef = useRef<AbortController | null>(null);
  const renderRef = useRef<AbortController | null>(null),
    downloadRef = useRef<AbortController | null>(null);
  ...
  const currentSignature = promptSignature(
    data.settings,
    data.notes,
    data.source?.saved_name || null,
  );
  ...
  function patch(next: Partial<WorkspaceState>) {
  ...
  async function changeSource(next: ImageChangePayload | null) {
    ...
    try {
      ...
      patch({ source: stored });
      clearPreview();
      setNotice("Đã lưu ảnh gốc. Dự án sẽ tự động lưu.");
    } catch (err) {
      ...
      if (!controller.signal.aborted)
        setError(
          err instanceof Error
            ? err.message
            : "Không thể lưu ảnh gốc. Ảnh tham chiếu trước đó đã được giữ lại.",
        );
    } finally {
  ...
  function makeVersion(text: string, signature: string): PromptVersion {
    return {
      ...
      notes: data.notes,
      sourceName: source?.name || "Không có ảnh",
  ...
  async function generatePrompt() {
    ...
    if (data.versions.length >= 200) {
      setError("Lịch sử đã có 200 prompt. Xóa phiên bản không cần trước khi tạo tiếp.");
      return;
    }
    const controller = new AbortController();
    ...
    try {
      const result = await apiRequest<PromptResponse>(
        "/api/generate-prompt",
        {
          ...
          body: JSON.stringify({
            ...toPromptRequest(data.settings, data.notes),
            reference_image_name: data.source.saved_name,
            mode: data.promptMode,
          }),
        },
        75000,
      );
      ...
      if (typeof result.prompt !== "string" || !result.prompt.trim())
        throw new Error("Dịch vụ chưa trả về prompt hợp lệ.");
      ...
      setData((current) => ({
        ...
        promptModel: result.model,
      }));
      setNotice(
        result.mode === "template"
          ? "Đã tạo prompt tiếng Việt theo thiết lập."
          : "Đã tạo prompt tiếng Việt bằng AI và lưu nhận xét.",
      );
    } catch (err) {
      if (!controller.signal.aborted)
        setError(err instanceof Error ? err.message : "Không thể tạo prompt.");
    } finally {
  ...
  async function renderImage() {
    if (
      ...
      return;
    if (data.renderVersions.length >= 200) {
      setError("Lịch sử đã có 200 ảnh. Xóa phiên bản không cần trước khi render tiếp.");
      return;
    }
    const controller = new AbortController();
    ...
    setNotice("");
    try {
      if (!(await persistence.saveProject()))
        throw new Error("Hãy lưu dự án thành công trước khi dựng ảnh.");
      if (controller.signal.aborted) return;
      const result = await requestRender(
        {
          ...
          reference_image_name: data.source.saved_name,
          settings: toPromptRequest(data.settings, data.notes),
      ...
      if (!result.url || !result.name || !result.width || !result.height)
        throw new Error("Dịch vụ chưa trả về ảnh hợp lệ.");
      ...
      setData((current) => ({
        ...current,
        renderVersions: [version, ...current.renderVersions],
        activeRenderId: version.id,
      }));
  ...
  function restoreRenderVersion(version: RenderVersion) {
    cancelRequests();
    clearPreview();
    setData((current) => restoreRender(current, version));
    setProjectRevision((v) => v + 1);
    setError("");
    setNotice(
      version.source
        ? "Đã mở đúng ảnh gốc, ảnh render và thiết lập của phiên bản."
        : "Phiên bản cũ không lưu ảnh gốc. Cần chọn lại ảnh để tiếp tục.",
    );
  }
  ...
  async function switchProject(id: string, discard = false) {
    ...
    if (await persistence.openProject(id, discard)) {
      clearPreview();
      setProjectRevision((v) => v + 1);
      setError("");
      setNotice("");
  ...
  async function resetProject() {
    ...
    if (success) {
      ...
      setProjectRevision((v) => v + 1);
      setError("");
      setNotice("Dự án trước đã được giữ trong danh sách.");
    } else {
      if (
        window.confirm(
          "Không thể lưu dự án hiện tại do lỗi đồng bộ. Bạn có chắc chắn muốn bỏ qua bản nháp này và tạo dự án mới không?",
        )
      ) {
        try {
          localStorage.removeItem(`infrarender.workspace.v2:${getBackendUrl()}`);
        } catch {}
  ...
  async function exportProject(snapshot: WorkspaceState = data) {
    ...
    if (!snapshot.source && snapshot.renderVersions.length === 0 && !snapshot.prompt.trim()) {
      setError("Dự án chưa có dữ liệu để xuất.");
      return;
    }
    setExporting(true);
  ...
  return {
    ...
    reloadProject: () => {
      if (
        ...
        window.confirm("Mở bản đã lưu trên máy chủ và bỏ các thay đổi chưa lưu trên thiết bị?")
      )
        void switchProject(persistence.projectId, true);
    },
    deleteCurrentProject: async () => {
      if (await persistence.deleteProject()) {
        clearPreview();
        setProjectRevision((v) => v + 1);
      }
    },
    clearPrompt: (mode: "prompt" | "negative") => {
      cancelRequests();
      patch(
        mode === "prompt"
          ? {
          ...
          : { negativePrompt: "", activeVersion: null },
      );
    },
    deleteVersion: (id: string) =>
      setData((current) => ({
        ...current,
        versions: current.versions.filter((v) => v.id !== id),
        activeVersion: current.activeVersion === id ? null : current.activeVersion,
      })),
    deleteRenderVersion: (id: string) => {
      setData((current) => ({
        ...current,
        renderVersions: current.renderVersions.filter((v) => v.id !== id),
        activeRenderId: current.activeRenderId === id ? null : current.activeRenderId,
      }));
      setNotice(
        "Đã bỏ khỏi lịch sử. Tệp không còn được sử dụng sẽ được dọn theo thời hạn lưu trữ.",
      );
    },
    ...
    saveVersion: () => {
      ...
      if (data.versions.length >= 200) {
        setError("Lịch sử đã đủ 200 prompt.");
        return;
      }
      const version = makeVersion(data.prompt, data.generatedFrom || currentSignature);
      setData((current) => ({
        ...current,
        versions: [version, ...current.versions],
        activeVersion: version.id,
    ...
    toggleFavorite: (id: string) =>
      setData((current) => ({
        ...current,
        versions: current.versions.map((v) => (v.id === id ? { ...v, favorite: !v.favorite } : v)),
      })),
