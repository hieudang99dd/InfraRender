...
export function useProjectPersistence(
  data: WorkspaceState,
  setData: Dispatch<SetStateAction<WorkspaceState>>,
  blocked: boolean,
) {
  ...
  const writeDraft = useCallback((workspace: WorkspaceState) => {
    try {
      localStorage.setItem(
        `infrarender.workspace.v2:${getBackendUrl()}`,
        JSON.stringify({ workspace, project: identity.current, synced: synced.current }),
      );
      setLocalError("");
    } catch {
      setLocalError(
        "Không lưu được bản nháp trên trình duyệt. Hãy kiểm tra trạng thái lưu máy chủ trước khi đóng trang.",
      );
    }
  }, []);
    ...
    async function load() {
      ...
      try {
        ...
        if (draft) {
          ...
          if (typeof parsed.project?.id === "string" && Number.isInteger(parsed.project.revision))
            identity.current = parsed.project;
          synced.current = typeof parsed.synced === "string" ? parsed.synced : "";
        } else {
          const legacy =
            localStorage.getItem("infrarender.workspace.v1") ||
            localStorage.getItem("infraRender_workspace");
          if (legacy)
            loaded = normalizeWorkspace(JSON.parse(legacy), DEFAULT_SETTINGS, getBackendUrl());
        }
      } catch {
        setLocalError(
          "Bản nháp cũ không hợp lệ hoặc trình duyệt không cho phép lưu. Các dự án trên máy chủ vẫn có thể mở.",
        );
      }
      try {
        const result = await apiRequest<{ projects: ProjectSummary[] }>("/api/projects", {
        ...
        if (identity.current && synced.current === JSON.stringify(loaded)) {
          const doc = await apiRequest<ProjectDocument>(`/api/projects/${identity.current.id}`, {
            signal: controller.signal,
          });
        ...
        if (!controller.signal.aborted)
          setSaveError(err instanceof Error ? err.message : "Chưa kết nối được kho dự án.");
      }
      if (controller.signal.aborted) return;
      current.current = loaded;
      setData(loaded);
      setProjectId(identity.current?.id || null);
      setSaveState(
        synced.current === JSON.stringify(loaded)
          ? "Đã lưu trên máy chủ"
      ...
      while (retryLoop) {
        ...
        try {
          while (alive.current && !busy.current) {
            ...
            if (
              !identity.current &&
              serialized === JSON.stringify(emptyWorkspace(DEFAULT_SETTINGS))
            )
              break;
            setSaveState("Đang lưu…");
            const previous = identity.current;
            const doc = await apiRequest<ProjectDocument>(
              previous ? `/api/projects/${previous.id}` : "/api/projects",
              {
                method: previous ? "PUT" : "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  name: snapshot.projectName.trim() || "Dự án chưa đặt tên",
                  workspace: snapshot,
            ...
            setProjects((list) => [
              { id: doc.id, name: doc.name, revision: doc.revision, updated_at: doc.updated_at },
              ...list.filter((p) => p.id !== doc.id),
            ]);
          ...
          if (alive.current) {
            ...
            conflictRef.current = collided;
            setConflict(collided);
            setSaveError(
              collided
                ? "Dự án đã thay đổi ở nơi khác hoặc ảnh tham chiếu không còn. Tải bản máy chủ hoặc lưu thành bản sao; bản nháp hiện tại vẫn được giữ."
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("infrarender-connection", retry);
    };
  ...
  async function openProject(id: string, discard = false) {
    ...
    try {
      ...
      setSaveError(err instanceof Error ? err.message : "Không thể mở dự án.");
      return false;
    }
  }
  ...
  async function deleteProject() {
    if (
      ...
      busy.current ||
      !window.confirm(
        "Xóa dự án này cùng các ảnh chỉ thuộc dự án này? Thao tác không thể hoàn tác.",
      )
    )
      return false;
    if (pending.current) await pending.current;
    try {
      ...
      setData(next);
      writeDraft(next);
      setSaveState("Đã xóa dự án");
      setSaveError("");
      await refreshProjects();
      return true;
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : "Không thể xóa dự án.");
      return false;
    }
  }
  return {
    ...
    refreshProjects: () =>
      refreshProjects().catch((err) =>
        setSaveError(err instanceof Error ? err.message : "Không thể tải danh sách dự án."),
      ),
  };
}
